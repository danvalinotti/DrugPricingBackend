...
public interface ReportRowRepository extends JpaRepository<ReportRow,Integer> {
    ...
    @Query(value= "SELECT " +
            ...
            " FROM " +
            " (SELECT  drug_master.name, price.price, price.program_id, drug_master.quantity, drug_master.dosage_strength  from report_drugs " +
            ...
            " WHERE report_id = ?1) t " +
            " GROUP BY t.name , t.dosage_strength, t.quantity" +
